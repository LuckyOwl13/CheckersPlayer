...
file = open("FinalBoardStates%s.txt" % str(threshold),"r")
...
x = torch.tensor(scale(np.array(XarrTemp), -1, 1)).type(torch.FloatTensor)
print(x.size())
print(x)
...
print("\nSaving model as SixLayerNet-%s.pth" % str(threshold))
torch.save(model.state_dict(), "models/SixLayerNet-%s.pth" % str(threshold))